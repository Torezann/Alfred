...
def main():
    ...
    if audio_value := st.experimental_audio_input("Diga algo"):
        try:
            file_audio = sr.AudioFile(audio_value)
            r = sr.Recognizer()

            with file_audio as source:
                texto = r.record(source)
                question = r.recognize_google(texto,language='pt-BR')
            ...
            with st.chat_message("assistant"):
                st.write("Não consegui entender, poderia repetir por gentileza?")
